...
function newsDetail() {
  ...
  function makeComment(comments) {
    var called = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
    var commentString = [];
...
function router() {
  ...
  if (routePath === '') {
    // location 해쉬에 #이 들어오면 빈 값 반환이 되어 조건문이 동작하는 것
    newsFeed();
  } else if (routePath.indexOf('#/page/') >= 0) {
    // 문자열 값 안에 존재하면 위치정보, 없으면 -1
